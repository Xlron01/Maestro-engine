...
### Changed
- **توحيد المستودع في نسخة واحدة (بتكليف المالك):** المشروع كله صار في  على فرع  وحيد — لا worktrees ولا فروع فرعية.
  - الأساس: الخط المعتمد t040-full-benchmark @8867491c (يشمل إعادة بناء TASK-040 الموثقة ebb0fde9). النسخة القديمة  حُذفت بالكامل بعد التحقق من صفر فقد.
  - نقل العمل المفيد: README overhaul من ae16c9c4 (d96be780) + بلوكات D1 milestone runs #7-#8 من التسليم المرفوض وworking dir القديم (e67ffb95).
  - التسليم المرفوض ee98f568: محفوظ **كتاريخ فقط** عبر merge استراتيجية ours (5f1a6f4a) — صفر تبنّى محتوى، الاعتماد الرسمي يظل لإعادة البناء النظيفة.
  - إصلاح 107 روابط مطلقة كانت تشير للمسار المحذوف → روابط نسبية (validate_memory: 0 errors).
...
- **Tranches A/B التنفيذية للنموذج:** `scripts/relevance_supply.gd` (Supply primitives: ExposureSupply/EoR/RF) + `scripts/relevance_control.gd` (Possession/Authority/DerivedPossession/C1–C4 مع visits/steps/cycle_events تشخيصية) + `relevance_access` (قناة الوصول) + عوالم `data/worlds/model_v1/` + عدّائات Tranche A/B/Integration.
...
### Tests
- **Test 1′ — Relevance Pipeline over FROZEN Model v1: PASS 20/20** — عالمان (ASML-pattern + هرمز-style مع net-exporter) يغطيان: ظهور القناتين (إمداد عبر إنتاج، وصول عبر سلسلة سلطة بلا إنتاج/اعتماد مباشر)، L1-joint bitwise تحت انقلاب نية، إغلاق بوابة/كسر سلطة ⇒ 0.0 بالضبط، حساسية احتياطي، net-exporter صفر كليًا. **اكتملت فجوة تنفيذ `ExposureTransit` (§3.2 المجمدة)** كجزء من التنفيذ الموثق. Evidence: `model_v1_test1prime_v2.log`.
- **Model v1 FROZEN — Integration Gate PASS 7/7 (§9.3):** القناتان + السلاسل على عالم موحد (ASML-pattern)؛ I-1 Supply mirror exact | I-2 dp=0.8 بالضبط | I-3 Access mirror exact | I-4 L1-joint bitwise تحت انقلاب نية/عداء | I-5 صفر مفاتيح تجميعية | I-6 float خالص | I-7 حتمية مزدوجة. ScenarioTest regression **5/5 + Checksum ثابت**. القيم: exposure=1.05، access=0.84، rel_supply=0.945، total=1.785. Evidence: `model_v1_integration.log` | `model_v1_freeze_scenariotest.log`.
